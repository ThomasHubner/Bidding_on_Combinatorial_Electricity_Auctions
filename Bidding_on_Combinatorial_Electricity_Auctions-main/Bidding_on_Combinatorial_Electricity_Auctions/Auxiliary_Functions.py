...
def bid_outcome(case_study, case_data, Time_set, real_price, bid, bid_type, Bid_set):
    ...
    bid_type : string
        What kind of group is it? (non-exsclusive, exclusive, linked)
    ...
    elif bid_type == "ungrouped":
        
        bundle = np.zeros(len(Time_set))
        for b in Bid_set:
            if profits[b] >= 0:
                bundle = bundle + np.array(bid["x"+str(b)])
    ...
    # Determine whether the bundle is a truthful bid for non-exclusive
    if bid_type == "ungrouped":
        
        total_profit = sum(profits[b] for b in Bid_set if profits[b]>=0)
        
        if abs(utility-total_profit) > 0.01:
            print("Untruthful bid")
            print("utility", utility, "total_profit", total_profit)
